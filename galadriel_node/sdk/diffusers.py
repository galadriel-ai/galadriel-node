...
class Diffusers:
    def __init__(self, model: str):
        try:
            self.pipeline = DiffusionPipeline.from_pretrained(model, torch_dtype=torch.float16)
            if torch.cuda.is_available():
                self.pipeline.to("cuda")
            elif config.GALADRIEL_ENVIRONMENT != "production" and torch.backends.mps.is_available():
                # Local test with Apple Silicon
                self.pipeline.to("mps")
                # Warm up the pipeline for CPU device usage
                _ = self.pipeline("a photo of the first snow in Tallinn", num_inference_steps=1)
            else:
                raise SdkError("CUDA is not available")
        except Exception as e:
            raise SdkError(f"Failed to initialize Diffusion pipeline: {e}")

    def generate_images(self, prompt: str, image: Optional[str] = None, n: int = 1) -> List[str]:
...
def _encode_image_to_base64(image: Image) -> str:
    buffer = io.BytesIO()
    image.save(buffer, format="PNG")
    return base64.b64encode(buffer.getvalue()).decode("utf-8")


def _decode_image_from_base64(image: str) -> Image:
